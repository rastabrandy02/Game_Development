...
bool Map::IsWalkable(int x, int y) const
{
	...
	if ((x >= 0 && x < data.width) && (y >= 0 && y < data.height) && (data.layers.start->next->data->Get(x, y) == 0)) isWalkable = true;
	return isWalkable;
...
void Map::PropagateBFS()
{
	...
	{
		iPoint pointNeighbours[4];
		pointNeighbours[0] = iPoint(point.x, point.y - 1);
		pointNeighbours[1] = iPoint(point.x - 1, point.y);
		pointNeighbours[2] = iPoint(point.x + 1, point.y);
		pointNeighbours[3] = iPoint(point.x, point.y + 1);
		for (int i = 0; i < 4 ; i++)
		{
			if (IsWalkable(pointNeighbours[i].x, pointNeighbours[i].y))
			{
				if (visited.Find(pointNeighbours[i]) == -1)
				{

					// L10: DONE 2: For each neighbor, if not visited, add it
					// to the frontier queue and visited list

					frontier.Push(pointNeighbours[i]);
					visited.Add(pointNeighbours[i]);
				}
			}
